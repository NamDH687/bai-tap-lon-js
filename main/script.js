...
function setSliderVisible(isVisible) {
  ...
  sliderBlock.style.setProperty("display", isVisible ? "block" : "none", "important");
...
document.addEventListener("click", function(e) {
    ...
    const rawAmount = amountInput ? amountInput.value : prompt("Nhập số tiền muốn nạp:");
...
document.addEventListener("click", function(e) {
    ...
    if (addCartBtn) {
        addToCart(addCartBtn.dataset.cartName, addCartBtn.dataset.cartPrice);
        return;
    }
...
function addToCart(name, price) {
    if (price === "Miá»…n phÃ­" || price === "Miễn phí") {
...
function parsePrice(price) {
    return Number(String(price).replaceAll(".", "").replace("đ", "").replace("Ä‘", ""));
}
    ...
    cart.forEach((item, index) => {
        ...
        cartItems.innerHTML += `
            <div class="d-flex justify-content-between align-items-center border-bottom py-2">
                ...
                <span>
                    ${item.price}
                    <button class="btn btn-sm btn-danger ms-2" onclick="removeCartItem(${index})">
                        X
                    </button>
                </span>
            </div>
